...
class USER: NSObject {
    ...
    //MARK: Inits
    init(name: String, email: String, id: String, profilePic: UIImage,latitude: Any,longitude: Any,fcmToken: String) {
        self.name = name
        self.email = email
        self.id = id
    ...
    //MARK:- registerUser
    class func registerUser(withName:String,email:String,password:String,profilePic:UIImage,fcmToken: String,notificationCount: Int,location: Dictionary<String, Any>,loginHandler: Loginhandler?){
        showLoader()
                            ...
                            if error == nil{
                                ...
                                Database.database().reference().child("users").child((AppModel.shared.currentUser.id)!).child("credentials").updateChildValues(values, withCompletionBlock: { (error, _) in
                                    if error == nil {
                                        ...
                                        UserDefaults.standard.set(AppModel.shared.currentUser.id, forKey: "currentUser")
    ...
    class func downloadAllUsers(exceptID: String, completion: @escaping (USER) -> Swift.Void) {
        Database.database().reference().child("users").observe(.childAdded, with: { (snapshot) in
            ...
            if id != exceptID {
                ...
                let fcmToken = credentials["fcmToken"] as? String
                let link = URL.init(string: credentials["profilePicLink"]! as! String)
                URLSession.shared.dataTask(with: link!, completionHandler: { (data, response, error) in
                    if error == nil {
                        let profilePic = UIImage.init(data: data!)
                        let user = USER.init(name: name as! String, email: email as! String, id: id, profilePic: profilePic!, latitude: latitude!, longitude: longitude!, fcmToken: fcmToken!)
                        completion(user)
                    }
                }).resume()
            }
